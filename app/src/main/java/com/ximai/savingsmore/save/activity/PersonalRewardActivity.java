...
public class PersonalRewardActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initEvent() {
         ...
         * 打开红包
         */
        personalRewardAdapter.setOnClickListener(new PersonalRewardAdapter.OnItemClickListener() {
            @Override
    ...
    private void initData() {
        personalRewardAdapter = new PersonalRewardAdapter(PersonalRewardActivity.this);
        recycle_view.setAdapter(personalRewardAdapter);
//        /**
//         * 获取红包列表
//         */
//        getRewardData();
            ...
            public void onResponse(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    if (0 == rewardList.size()) {
                        ...
                        personalRewardAdapter.setDdata(rewardList);
                        personalRewardAdapter.notifyDataSetChanged();
                    }
        ...
        WebRequestHelper.json_post(this, URLText.PLAY_REWARD, RequestParamsPool.playReward(Id,Remark), new MyAsyncHttpResponseHandler(this) {
            ...
            public void onResponse(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    if ("现金返利正在审核中！".equals(message)){
                        apply();
                    }

                    if (true == isSuccess){
                        Toast.makeText(PersonalRewardActivity.this, "打开成功", Toast.LENGTH_SHORT).show();
                        //在这里进行红包打开的操作 - 不进行列表的刷新的 - 直接让item进行变换
                        rewardList.get(position).setOpen(true);
                        personalRewardAdapter.notifyItemChanged(position);
                        //更新价格
                        updataMoney();
                    }
                } catch (JSONException e) {
    ...
    public void apply(){
        ...
        Dialog dialog = new XiMaiPopDialog(this, "温馨提示", "返利申请正在审核中，请稍后", "确认", R.style.CustomDialog_1, callBack, 2);
...
//                        AndPermission.rationaleDialog(mActivity, rationale).show();
                        com.yanzhenjie.alertdialog.AlertDialog.newBuilder(PersonalRewardActivity.this)
                                .setTitle("友好提醒")
                ...
                .callback(new PermissionListener() {
                    ...
                    public void onFailed(int requestCode, List<String> deniedPermissions) {
                        if (AndPermission.hasAlwaysDeniedPermission(PersonalRewardActivity.this, deniedPermissions)) {
                            AndPermission.defaultSettingDialog(PersonalRewardActivity.this, 400)
                                    .setTitle("权限申请失败")
                        ...
                        if (requestCode == 200) {// Failure.
                            Toast.makeText(PersonalRewardActivity.this, "请去设置打开拨打电话权限", Toast.LENGTH_SHORT).show();
                        }
                    }
                })
                .start();
