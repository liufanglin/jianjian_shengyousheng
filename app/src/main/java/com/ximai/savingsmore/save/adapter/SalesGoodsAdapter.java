...
public class SalesGoodsAdapter extends RecyclerView.Adapter<SalesGoodsAdapter.ViewHolder>{

    private Context context;
    private List<Goods> list = new ArrayList<>();
    private OnItemClickListener onItemClickListener;

    public SalesGoodsAdapter(Context context) {
...
//            holder.tv_business.setText(list.get(position).StoreName);
            if (null != list.get(position).ChainStores){//---------------------------------------------------------进行分店的实现
                ...
                if (null != list.get(position).StoreName) {//商家店铺名称
                    holder.tv_business.setText(list.get(position).StoreName);//商家名称
                }
            }
...
//            holder.end_time.setText(list.get(position).EndTimeName.substring(0,10));
    ...
    public void setViewClickListener(SalesGoodsAdapter.OnItemClickListener listener){
