package com.ximai.savingsmore.save.view.imagepicker.adapter;
...
public class PhotoSelectorAdapter extends MBaseAdapter<PhotoModel> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		RelativeLayout tvCamera = null;
		//这里设置第一个是否可以进行拍照 - 下面的position是根据第一个是否显示来确定是否加1
		if (position == 0 && StringUtils.isNull(models.get(position).getOriginalPath())) { // 当时第一个时，显示按钮
			if (convertView == null || !(convertView instanceof RelativeLayout)) {
				tvCamera = (RelativeLayout) LayoutInflater.from(context).inflate(R.layout.view_camera, null);
				tvCamera.setMinimumHeight(itemWidth);
				tvCamera.setMinimumWidth(itemWidth);
