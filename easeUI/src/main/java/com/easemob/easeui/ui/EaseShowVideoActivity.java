...
public class EaseShowVideoActivity extends EaseBaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		EMLog.d(TAG, "show video view file:" + localFilePath + " remotepath:" + remotepath + " secret:" + secret);
...
//			finish();
            ...
            if (Build.VERSION.SDK_INT >= 24) {
                //如果是7.0或以上
                uriForFiles = FileProvider.getUriForFile(this, getPackageName() + ".fileprovider", new File(localFilePath));
            } else {
                uriForFiles = Uri.fromFile(new File(localFilePath));
            }
            intent.setDataAndType(uriForFiles, "video/mp4");
            startActivity(intent);
            finish();
	...
	private void showLocalVideo(String localPath){
		...
		if (Build.VERSION.SDK_INT >= 24) {
			//如果是7.0或以上
            uriForFile = FileProvider.getUriForFile(this, getPackageName() + ".fileprovider", new File(localPath));
        } else {
            uriForFile = Uri.fromFile(new File(localPath));
		}
